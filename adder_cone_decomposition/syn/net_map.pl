...
my $gate_pattern = "XOR2D1|XNOR2D0|XOR2D0|CKXOR2D1|CKXOR2D1HVT|ND2D0|INVD0|OR2D1|OR2D0|NR2D0|AN2D0|CLKBUF_X1";
...
while($original_nl = <CKT_TEMP>)
{
	...
	elsif($original_nl =~ /($gate_pattern)/)
	{
		...
		elsif($gate_type eq 'OR2D0' || $gate_type eq 'OR2D1')
		{
			$original_nl =~ /\.\w+\s*\((\w+)\)\s*,\s*\.\w+\s*\((\w+)\)\s*,\s*\.\w+\s*\((\w+)\)/;
			$left = $1;
			$right = $2;
		...
		elsif($gate_type eq 'XOR2D0' || $gate_type eq 'XOR2D1' )
		{
			$original_nl =~ /\.\w+\s*\((\w+)\)\s*,\s*\.\w+\s*\((\w+)\)\s*,\s*\.\w+\s*\((\w+)\)/;
			$left = $1;
			$right = $2;
